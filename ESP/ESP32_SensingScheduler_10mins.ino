...
RTC_DATA_ATTR String ExecutiveSchedule[6] = {"12", "5", "300", "4", "5", "2000"};
...
void ParseMessage(String RecievedMessage)
{
  ...
  else
  {
    ...
    if (Position[0] == -1 || RecievedMessage.length() <= 18)
    {
      ...
      ExecutiveSchedule[1] = "5";
      ExecutiveSchedule[2] = "300";
      ExecutiveSchedule[3] = "4";
      ExecutiveSchedule[4] = "5";
...
void valve()
{
  ...
  {
    int minutes_digits = (minutes.toInt())%10;
    if (minutes_digits == ExecutiveSchedule[3].toInt())
    {
...
void loop() {
  ...
  int minutes_digits = (minutes.toInt())%10;
  if (minutes_digits == ExecutiveSchedule[1].toInt())
  {
  ...
  else if (FlagOfValve == true && Acc_Time > 600000)
  {
    digitalWrite(relay_Sensor, LOW);//繼電器常開
    digitalWrite(relay_valve, LOW);//繼電器長開
